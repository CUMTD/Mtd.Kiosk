...
public sealed class AdafruitSensorWorker : TemperatureMonitorBackgroundService
{
	...
	private const string QUERY_STRING = "SELECT * FROM Win32_PnPEntity WHERE Caption LIKE '%(COM%'";
	...
	protected override async Task<(int temperature, int humidity, int sensorType)> ReadDataFromSensor(CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			var last = separated.Last();
			return (last.Item1, last.Item2, _sensorConfig.SensorType);

		}
		catch (Exception ex)
		{
			_logger.LogError(ex, "Serial port error");
			await HandlePortDisconnectionAsync(cancellationToken);
			throw;
		}
	}
	...
	{
		var searcher = new ManagementObjectSearcher(QUERY_STRING);
	...
	private async Task HandlePortDisconnectionAsync(CancellationToken token)
	{
		_serialPort?.Close();
		...
		{
			_logger.LogError("Reconnection is only supported on Windows.");
		}
	}
}
