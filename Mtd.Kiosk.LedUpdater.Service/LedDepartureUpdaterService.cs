...
internal class LedDepartureUpdaterService : BackgroundService, IHostedService, IDisposable
{
	...
	protected readonly LedRealtimeClient _realtimeClient;
	...
	public LedDepartureUpdaterService(
			...
			LedRealtimeClient realtimeClient,
	...
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		...
		{
			var activeKioskMessage = await FetchGeneralMessagesAsync(stoppingToken);
			...
			if (_departuresStack.Count == 0)
			{
				...
				var updateResult = await UpdateDeparturesAsync(stoppingToken);
				if (!updateResult)
				{
			...
			{
				await _realtimeClient.LogHeartbeatAsync(Kiosk.Id, stoppingToken);
			}
	...
	private async Task<bool> UpdateDeparturesAsync(CancellationToken cancellationToken)
	{
		var departures = await FetchDeparturesAsync(cancellationToken);
	...
	private async Task<IReadOnlyCollection<Departure>?> FetchDeparturesAsync(CancellationToken cancellationToken)
	{
		if (Kiosk == null)
		{
			return default;
	...
	private async Task<GeneralMessage?> FetchGeneralMessagesAsync(CancellationToken cancellationToken)
	{
		if (Kiosk == null)
		{
			return default;
