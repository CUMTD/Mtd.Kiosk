...
internal class LedBrightnessService : BackgroundService, IHostedService, IDisposable
{
	...
	private readonly LedRealtimeClient _realtimeClient;
	...
	public LedBrightnessService(IOptions<LedUpdaterServiceConfig> config, LedRealtimeClient realtimeClient, IpDisplaysApiClientFactory ipDisplaysClientFactory, SanityClient.SanityApiClient sanityApiClient, ILogger<LedBrightnessService> logger)
	{
	...
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		...
		{
			var darkModeStatus = await FetchDarkModeStatusAsync(stoppingToken);
	...
	private async Task<bool> FetchDarkModeStatusAsync(CancellationToken stoppingToken)
	{
		try
		{
			var darkMode = await _realtimeClient.GetDarkModeStatusAsync(stoppingToken);
