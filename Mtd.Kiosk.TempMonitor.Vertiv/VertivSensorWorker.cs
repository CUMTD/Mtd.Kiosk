...
using Mtd.Kiosk.TempMonitor.Vertiv.Model;
...
public sealed class VertivSensorWorker : TemperatureMonitorBackgroundService
{
	...
	protected override async Task<(int temperature, int humidity, int sensorType)> ReadDataFromSensor(CancellationToken cancellationToken)
	{
		...
		var deserializedResponse = JsonSerializer.Deserialize<SensorResponse>(json) ?? throw new Exception("No data in Vertiv sensor response.");
		...
		{
			return (temperature.Value, humidity.Value, _sensorConfig.SensorType);
		}
