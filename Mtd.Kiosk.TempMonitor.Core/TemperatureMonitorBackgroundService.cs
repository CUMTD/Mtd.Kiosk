...
public abstract class TemperatureMonitorBackgroundService : BackgroundService, IHostedService
{
	...
	private async Task RunMonitorLoop(CancellationToken cancellationToken)
	{
		while (!cancellationToken.IsCancellationRequested)
		{
			...
			try
			{
				(temperature, humidity, sensorType) = await ReadDataFromSensor(cancellationToken);
			...
			try
			{
				await SendTemperatureToApi(temperature, humidity, sensorType, cancellationToken);
			}
			catch (Exception ex)
			{
	...
	protected abstract Task<(int temperature, int humidity, int sensorType)> ReadDataFromSensor(CancellationToken cancellationToken);

	private async Task SendTemperatureToApi(int temperature, int humidity, int sensorType, CancellationToken cancellationToken)
	{
		var uri = new Uri($"{_config.DataCollectionEndpoint}/{_config.KioskId}?temp={temperature}&humidity={humidity}&sensorType={sensorType}");
